...
const INDEXNAME   = constants.INDEXNAME;
const MaxCSVRows  = constants.MAXCSVROWS;
...
export default class CsvGeneratorService {
 ...
 updateCSV = async (_req, documentId, status, binary, error) => {
  ...
  genereteCsv = async (_req, documentId) => {
    ...
    for(const item of savedSearchInfos._source.references) {
      ...
      if (item.name === JSON.parse(filters).indexRefName) {
        ...
        const sorting = '"sort":[{"' + strSort[0][0] + '":{ "order": "' +  strSort[0][1] + '"} }] ';
        // console.log('sorting is ', sorting);
        // console.log('strSort[0][0] is ', strSort[0][0]);
        // console.log('strSort[0][1] is ', strSort[0][1]);
        if(fields_exist) {
          body            = '{ "version": true, "size": 1000,' + sorting + ', "_source": { "includes": [' + strColumns + '] },' + body + '}';
        }else{
          body            = '{ "version": true, "size": 1000,' + sorting + ',' + body + '}';
        }

        // body            = '{ "version": true, "size": 1000,' + sorting + ', "_source": { "includes": [' + strColumns + '] },' + body + '}';
        //Count
        console.log('body is', body);
        const resCount  = await this.esFetchCount(_req, resIndexPattern.title, bodyCount).catch(err =>{
          this.updateCSV(_req, documentId, 'failed', nullBinary, 'Err While Fetching the count in ES');
        });
        ...
        for(const valueRes of arrayHits) {
          console.log('valueRes is ', valueRes);
  ...
  createPendingCsv = async (_req) => {
    ...
    const time_range_lte   = _req.params.end;
    const savedsearchId    = _req.params.savedsearchId;
    console.log('time_range_gte is ', time_range_gte);
    console.log('time_range_lte is ', time_range_lte);
    console.log('MaxCSVRows is ', MaxCSVRows);
