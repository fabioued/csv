...
export class Main extends React.Component {
  ...
  generateProxy = () => {
    if(this.state.clickCount >= config.CLICKLIMIT) {
  ...
  generateCsv = () => {
    ...
    //api url
    const url =
      '../api/csvGenerator/savedObjects/' + savedSearchId + '/' + startMoment + '/' + endMoment;
    this.setState({
      hideLoader: false,
      hideCsvItem: true,
      buttonIsLoading: true,
    });
    let generate= httpClient.get(url);
    console.log('generate is ', generate);
    setTimeout(() => {
      this.refreshRecevntCsv();
    }, 1000);
    this.setState({
      hideLoader: true,
      buttonIsLoading: false,
    ...
    //setimeout 2s then cqll the refresh function
    //kill the loading
  ...
  componentDidMount() {
    ...
    const { httpClient } = this.props;
    this.getSavedSearch();
    console.log('constants are ',config);
    console.log('httpClient from main is ', httpClient);
  }
          ...
          <EuiPageContent>
            ...
            <EuiPageContentBody>
              ...
              {this.state.hideCsvItem ? null : (
                <CsvItem
                  title="List of Generated CSV"
                  items={this.state.recentCsv}
                  refresh={this.refreshRecevntCsv}
